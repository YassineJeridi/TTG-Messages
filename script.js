...
function buildProgress() {
  const s = document.getElementById("progress-strip");
  s.innerHTML = "";
...
function buildFlowWithPhase3() {
  ...
  D.phases.forEach((item, i) => {
    if (item.isVariants) {
      ...
      card.addEventListener("click", () => openSuggestionModal(i));
      c.appendChild(card);
    } else {
...
function mkCard(item, idx, section) {
  ...
  card.addEventListener("click", () => {
    ...
    else if (item.hasToggleX || item.hasChooseY)
      openQAModal(item, idx, section);
    else
      copy(
        item.message,
        `${section}-card-${idx}`,
        section === "flow" ? idx : null,
      );
  });
  return card;
}
...
function openOptionsModal(item, idx, section) {
  ...
  curIdx = idx;
  curSection = section;
  document.getElementById("opt-preview").textContent =
    item.base.slice(0, 140) + "...";
  openM("options-modal");
}
function pickOption(n) {
  const msg = curItem.base + (n === 1 ? curItem.option1 : curItem.option2);
  closeAll();
  copy(msg, `${curSection}-card-${curIdx}`, curIdx);
}

function openPhase3Modal(phaseIdx) {
  const wrap = document.getElementById("p3-opts");
  wrap.innerHTML = "";
...
function openSuggestionModal(phaseIdx) {
  const wrap = document.getElementById("sug-opts");
  wrap.innerHTML = "";
  D.suggestionVariants.forEach((v) => {
    const b = document.createElement("button");
    b.className = "opt-btn";
    b.innerHTML = `<span class="opt-label-s">${v.tag}</span><span class="opt-txt">${v.label}</span>`;
    b.onclick = () => {
      wrap
        ...
        .forEach((x) => x.classList.remove("selected"));
      b.classList.add("selected");
      document.getElementById("sug-preview").textContent = v.message;
      document.getElementById("sug-copy-btn").onclick = () => {
        closeAll();
        copy(v.message, `flow-card-${phaseIdx}`, phaseIdx);
      };
    };
...
function openPriceModal(item, idx, section) {
  ...
  curIdx = idx;
  curSection = section;
  const wrap = document.getElementById("price-opts");
  wrap.innerHTML = "";
  item.prices.forEach((p) => {
    ...
    b.onclick = () => {
      const msg = item.base_before + p + item.base_after;
      closeAll();
      copy(msg, `${section}-card-${idx}`, null);
    };
    wrap.appendChild(b);
  });
  openM("price-modal");
...
function openPrefixModal(item, idx, section) {
  ...
  const wrap = document.getElementById("pfx-opts");
  wrap.innerHTML = "";
...
function updatePfxPreview() {
  if (!curItem) return;
  const msg =
    activePfx && activePfx !== "none"
      ? activePfx + " " + curItem.base
      : curItem.base;
...
function doPrefixCopy() {
  ...
  copy(
    msg,
    `${curSection}-card-${curIdx}`,
    curSection === "flow" ? curIdx : null,
  );
}
...
function openSuffixModal(item, idx, section) {
  ...
  document.getElementById("sfx-toggle").textContent =
    "⚠️ Add confirmation warning — OFF";
...
function toggleSuffix() {
  ...
  btn.textContent = activeSuffix
    ? "⚠️ Add confirmation warning — ON ✅"
    : "⚠️ Add confirmation warning — OFF";
  document.getElementById("sfx-preview").textContent =
    curItem.base + (activeSuffix ? curItem.suffix : "") + curItem.ending;
}
...
function openQAModal(item, idx, section) {
  ...
  if (item.hasChooseY) {
    ...
    const wrap = document.getElementById("qa-y-opts");
    wrap.innerHTML = "";
...
function updateQAPreview() {
  ...
  if (curItem.hasChooseY) {
    if (activeY && activeY !== "none") msg += "\n\n" + activeY;
  } else {
    msg += curItem.ending;
  }
...
function doQACopy() {
  ...
  closeAll();
  copy(msg, `${curSection}-card-${curIdx}`, null);
}
...
function copy(text, cardId, phaseIdx) {
  const done = () => {
    toast();
    flash(cardId);
    if (phaseIdx != null) updateProgress(phaseIdx);
  };
  if (navigator.clipboard && window.isSecureContext)
    navigator.clipboard
      .writeText(text)
      .then(done)
      .catch(() => fbCopy(text, done));
...
function flash(id) {
  ...
  setTimeout(() => c.classList.remove("copied"), 1800);
}
let tt;
function toast() {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("opt1-btn")
    .addEventListener("click", () => pickOption(1));
  document
    .getElementById("opt2-btn")
    .addEventListener("click", () => pickOption(2));
  document
    .getElementById("pfx-copy-btn")
    .addEventListener("click", doPrefixCopy);
  document.getElementById("sfx-toggle").addEventListener("click", toggleSuffix);
  document
    .getElementById("sfx-copy-btn")
    .addEventListener("click", doSuffixCopy);
